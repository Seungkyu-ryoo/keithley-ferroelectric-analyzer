...
def parse_cycle_from_name(path):
    ...
    cycle_match = re.search(
        r"(?<![a-z0-9])cycle(?:_|-|\s)*(\d+(?:\.\d+)?(?:e[+-]?\d+)?)(?![a-z0-9])",
        stem,
    )
    if cycle_match:
        return cycle_text_to_number(cycle_match.group(1))
    cycle_match = re.search(
        r"(?<![a-z0-9])(\d+(?:\.\d+)?(?:e[+-]?\d+)?)(?:_|-|\s)*cycle(?![a-z0-9])",
        stem,
    )
    if cycle_match:
        return cycle_text_to_number(cycle_match.group(1))
    bare_cycle_match = re.fullmatch(r"\d+(?:\.\d+)?(?:e[+-]?\d+)?", stem)
    if bare_cycle_match:
        return cycle_text_to_number(bare_cycle_match.group(0))
    return None
...
def files_for_mode(base_folder, mode):
    ...
    candidates = []
    seen = set()
    for path in find_excel_files(base):
...
class FerroCycleQtApp(QMainWindow):
    ...
    def build_ui(self):
        ...
        self.alpha_start_spin = self.double_spin(0.25, 0.0, 1.0, 2)
        self.alpha_end_spin = self.double_spin(0.95, 0.0, 1.0, 2)
        self.alpha_start_spin.setSingleStep(0.05)
        self.alpha_end_spin.setSingleStep(0.05)
        alpha_row.addWidget(QLabel("Alpha start"))
        alpha_row.addWidget(self.alpha_start_spin)
        alpha_row.addWidget(QLabel("end"))
        alpha_row.addWidget(self.alpha_end_spin)
        ...
        color_layout.addLayout(alpha_row)
    ...
    def refresh_files(self):
        ...
        for path in files:
            ...
            if cycle is None:
                continue
            if cycle in self.cycle_file_map:
        ...
        for cycle in self.available_cycles:
            ...
            self.cycle_list.addItem(item)
            item.setSelected(True)
        status = f"{mode}: {len(self.available_cycles)} cycle files found"
        if duplicate_cycles:
            status += f"; {len(duplicate_cycles)} duplicate cycle files skipped"
    ...
    def color_cycle(self, n):
        ...
        if self.color_mode_combo.currentText() == "Single + alpha":
            ...
            start_alpha = self.alpha_start_spin.value()
            end_alpha = self.alpha_end_spin.value()
            if n == 1:
                return [((*base, 1.0), end_alpha)]
            return [
                ((*base, 1.0), start_alpha + (end_alpha - start_alpha) * i / (n - 1))
                for i in range(n)
            ]
